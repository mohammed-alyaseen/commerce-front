...
export default function DeleteItemButton({ item }: { item: CartItem }) {
  ...
  async function handleRemove() {
    ...
    setCart?.({
      lines: cart.lines.filter((line: CartItem) => line.merchandise?.product?.id !== item?.merchandise?.product?.id),
      cost: {
        subtotalAmount: {
          amount: Number(cart?.cost?.totalAmount?.amount) || 0 - Number(item?.merchandise?.product?.priceRange?.maxVariantPrice?.amount) || 0,
          currencyCode: 'USD'
        },
        totalTaxAmount: {
          amount: "0",
